...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6 col-xs-12">
            <form class="form-horizontal" action="{{ route('system.menu.store') }}" method="post" enctype="multipart/form-data">
                {!! $ui::box_start() !!}
                {!! $ui::box_header(trans('yeoman.menu.create_menu')) !!}
                    ...
                    <div class="col-sm-9">
                        <select id="parent_id" name="parent_id" class="select2_single form-control" tabindex="-1">
                            ...
                            @foreach($tree as $menu)
                                <option value="{{$menu->id}}" @if(old('parent_id') == $menu->id) selected @endif >{{$menu->html}} {{$menu->display_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <div class="radio">
                            <label>
                                <div class="iradio_flat-green" ><input class="flat" name="is_system" value="1" type="radio"></div>{{ trans('yeoman.bool.yes') }}
                            </label>
                            <label>
                                <div class="iradio_flat-green" ><input class="flat" name="is_system" value="0" checked type="radio"></div>{{ trans('yeoman.bool.no') }}
                            </label>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <div class="radio">
                            <label>
                                <div class="iradio_flat-green" ><input class="flat" name="is_display" value="1"  checked  type="radio"></div>{{ trans('yeoman.bool.yes') }}
                            </label>
                            <label>
                                <div class="iradio_flat-green" ><input class="flat" name="is_display"  value="0"  type="radio"></div>{{ trans('yeoman.bool.no') }}
                            </label>
                        </div>
                    </div>
                </div>
