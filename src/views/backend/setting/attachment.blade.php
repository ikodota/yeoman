...
@section('content')
    ...
    <div class="row">
        ...
        {!! $ui::box_start() !!}
        {!! $ui::box_header(trans('yeoman.setting.upload_setting')) !!}
...
@section('body_dialog')
    @include('yeoman::backend.modal.alert',['model_id'=>'success','model_title'=>'对话框','model_content'=>'测试结果：成功'])
    @include('yeoman::backend.modal.alert',['model_id'=>'fail','model_title'=>'对话框','model_content'=>'测试结果：失败'])
@stop
    ...
    <script>
        ...
        $('#btnQiniuTest').click(function() {
            ...
            $.getJSON('/admin/setting/attachment/test/qiniu', {
                ...
                //alert(data);
                if(status){
                    $('#alertModal_success').modal()
                }else{
                    $('#alertModal_fail').modal()
                }
